    ...
    private func handleNoiseEncrypted(_ packet: BitchatPacket, from peerID: PeerID) {
        ...
        do {
            ...
            case .alert:
                let ts = Date(timeIntervalSince1970: Double(packet.timestamp) / 1000)
                notifyUI { [weak self] in
                    self?.delegate?.didReceiveNoisePayload(from: peerID, type: .alert, payload: Data(payloadData), timestamp: ts)
                }
            case .sighting:
                let ts = Date(timeIntervalSince1970: Double(packet.timestamp) / 1000)
                notifyUI { [weak self] in
            ...
            case .dtnBundle:
                let ts = Date(timeIntervalSince1970: Double(packet.timestamp) / 1000)
                notifyUI { [weak self] in
                    self?.delegate?.didReceiveNoisePayload(from: peerID, type: .dtnBundle, payload: Data(payloadData), timestamp: ts)
                }
            case .dtnReceipt:
                let ts = Date(timeIntervalSince1970: Double(packet.timestamp) / 1000)
                notifyUI { [weak self] in
                    self?.delegate?.didReceiveNoisePayload(from: peerID, type: .dtnReceipt, payload: Data(payloadData), timestamp: ts)
                }
            case .dtnSummary:
                let ts = Date(timeIntervalSince1970: Double(packet.timestamp) / 1000)
                notifyUI { [weak self] in
                    self?.delegate?.didReceiveNoisePayload(from: peerID, type: .dtnSummary, payload: Data(payloadData), timestamp: ts)
                }
            case .none:
