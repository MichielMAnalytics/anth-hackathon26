# SafeThread — Architecture reference

Deep technical reference for the SafeThread platform. For the product overview, user story, and quick start, see [`README.md`](README.md). For the human-language user-story diagrams, see [`visuals/README.md`](visuals/README.md).
...
## 3. Backend — five nodes, four DB-coupled stages
...
**API tier (`server/api/*`).** Stateless ingress. `current_operator` dep reads `X-Operator-Id` against a static registry (full JWT auth deferred). FastAPI routers grouped by purpose, all listed in §6.
...
| GET | `/api/incidents` | active alerts shaped for the case grid (includes `replyCode`) |
...
| Method | Path | Effect |
|---|---|---|
| POST | `/api/incidents` | create a new case; allocates a 4-char `reply_code` |
...
All three broadcast endpoints persist a `ToolCall(decision_id=NULL, decided_by=<op>, approval_status='approved')` + an `OutboundMessage` placeholder, mirroring the agent path.

### Twilio SMS
| Method | Path | Effect |
|---|---|---|
| POST | `/webhooks/twilio/sms` | inbound civilian SMS. Validates `X-Twilio-Signature`, parses any leading 4-char reply code → routes onto matching active `Alert` via `in_reply_to_alert_id`, persists `InboundMessage`. |
...
| `Alert` | active case; carries `category`, `urgency_tier`, `urgency_score`, region, **`reply_code`** (4-char civilian-facing code for SMS threading) |
| `AlertDelivery` | denormalized roster of who received which alert |
| `InboundMessage` | raw inbound (queue: `new`/`triaging`/`triaged`/`failed`); `in_reply_to_alert_id` set when threaded onto a case |
...
**Backend.** Python 3.12+, FastAPI 0.110+, uvicorn, SQLAlchemy 2.0 async + asyncpg, alembic, pgvector, pydantic-settings, python-ulid, anthropic (bare client) for triage, claude-agent-sdk ≥ 0.1.68 for the agent worker (requires Claude Code CLI on PATH for real mode), pytest + pytest-asyncio + httpx, ruff, uv, twilio (SMS).
...
| 6 — SMS round-trip | Twilio outbound + inbound webhook, reply-code threading onto cases, live case-thread updates | shipped |
| 7 — Outbound dispatcher | real push, channel cascade, audience resolution, rate-limited token buckets | not yet started |
| 8 — Bitchat BLE on hub | bleak-backed BLE radio, real GATT service binding | stretch goal |
